...
install_if_changed "$BUILD_DIR/pfwd-xdp-linux-amd64" "$ROOT_DIR/dist/pfwd-xdp-linux-amd64" 0755

CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -trimpath -ldflags="-s -w -buildid=" -o "$BUILD_DIR/pfwd-xdp-linux-arm64" .
install_if_changed "$BUILD_DIR/pfwd-xdp-linux-arm64" "$ROOT_DIR/dist/pfwd-xdp-linux-arm64" 0755
