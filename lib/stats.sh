...
stats_rollup_counters() {
    ...
    stats_update --slurpfile snap "$tmp" --slurpfile cfg "$PFWD_CONFIG_FILE" '
      ...
      | reduce ($snap[0] | group_by(.user_id)[]?) as $group (.;
          ...
          ($old.billing_offset_bytes // (($old.billing_used_bytes // 0) - user_forward_sum($state; $user_id))) as $offset |
          .users[$user_id] = ($old + {
            billing_offset_bytes: $offset,
            billing_used_bytes: ((user_forward_sum(.; $user_id) + $offset) | if . < 0 then 0 else . end),
